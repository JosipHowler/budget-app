    ...
    return (
        <Card className={classNames.join(" ")}>
            <Card.Body>
                ...
                {max && (<ProgressBar className="rounded-pill" variant={getProgressBarColor(amount, max)} min={0} max={max} now={amount} />)}
                {!hideButtons && (<Stack direction="horizontal" gap="2" className="mt-4">
                    <Button variant="outline-primary" size="lg" className="ms-auto" onClick={onAddExpenseClick}>Add Expense</Button>
                    <Button onClick={onViewExpensesClick} variant="outline-secondary" size="lg">View Expense</Button>
                </Stack>)}
            </Card.Body>
        </Card>
    )
